...
class ScaDevice:
    def __init__(self, hw):
        self.hw = hw
        self.io_reg_base = "sca_io_dev."
        self.refclk_base = "sca_refclk_dev."
        self.dffclk_base = "sca_dffclk_dev."
        log.info("SCA Device")
    ...
    def drp_calc_m(self, divide, phase):
        ...
        if divide == 1:
            ...
            log.info("daddr_{:#2x}: {:#4x}\t-clkfbout register 1- ".format(daddr_reg1, drp_reg1))
            log.info("daddr_{:#2x}: {:#4x}\t-clkfbout register 2- ".format(daddr_reg2, drp_reg2))
    ...
    def drp_calc_d(self, divide):
        ...
        if divide == 1:
            ...
            log.info("DADDR_0x16: {:#4x}\t-DIVCLK Register $divide-".format(div_reg))
            return div_reg
        elif divide > 19:
            log.error("ERROR: DADDR_16: ERROR D must be 1 to 19")
            log.error("ERROR: 16 ERROR")
            sys.exit(-1)
        else:
            div_reg = ((ht & 0x3f) << 6) + (lt & 0x3f)
            log.info("DADDR_16: {:#04x}\t-DIVCLK Register {}-".format(div_reg, divide))
            return div_reg
    ...
    def drp_cpres(self, div, bw):
        ...
        filt_reg2 = (int(LFHF[1], base=2) << 4) + (int(LFHF[0], base=2) << 7) + (int(RES[3], base=2) << 8) + (
                int(RES[1:3], base=2) << 11) + (int(RES[0], base=2) << 14)
        log.info("DADDR_4E: {:#04x}\t-Filter Register 1: M set to {} with {} bandwidth-".format(filt_reg1, div, bw))
        log.info("DADDR_4F: {:#04x}\t-Filter Register 2: M set to {} with {} bandwidth-".format(filt_reg2, div, bw))
    ...
    def drp_locking(self, div):
        ...
        log.info("DADDR_28: FFFF\t-Power register leaving all interpolators on - ")
        log.info("DADDR_18: {:#04x}\t-Lock Register 1: for M set to {:d} -".format(lock_reg1, div))
        log.info("DADDR_19: {:#04x}\t-Lock Register 2: for M set to {}".format(lock_reg2, div))
        log.info("DADDR_1A: {:#04x}\t-Lock Register 3: for M set to {}".format(lock_reg3, div))
        powerReg = int(0xFFFF)
        return powerReg, lock_reg1, lock_reg2, lock_reg3

    def drp_clkout(self, divide, dutycycle, phase, clkout="clkout0"):
        """
        :param divide:
        ...
        """
        clkout_lower = clkout.lower()
        daddr_reg1 = 0x08
        daddr_reg2 = 0x09
        if clkout_lower == "clkout0":
            daddr_reg1 = 0x08
            daddr_reg2 = 0x09
        elif clkout_lower == "clkout1":
            daddr_reg1 = 0x0a
            daddr_reg2 = 0x0b
        elif clkout_lower == "clkout2":
            daddr_reg1 = 0x0c
            daddr_reg2 = 0x0d
        elif clkout_lower == "clkout3":
            daddr_reg1 = 0x0e
            daddr_reg2 = 0x0f
        elif clkout_lower == "clkout4":
            daddr_reg1 = 0x10
            daddr_reg2 = 0x11
        elif clkout_lower == "clkout5":
            daddr_reg1 = 0x06
            daddr_reg2 = 0x07
        elif clkout_lower == "clkout6":
            daddr_reg1 = 0x12
            daddr_reg2 = 0x13
        ...
        odd = divide - even_high * 2
        if divide == 1:
            drp_reg1 = ((pmphasecycles & 0x7) << 13) + 0b1000001000001
            drp_reg2 = (odd << 7) + (1 << 6) + (phasecycles & 0x3f)
            log.info("daddr_{}: {:#04x}\t-{} register 1- ".format(daddr_reg1, drp_reg1, clkout.upper()))
            log.info("daddr_{}: {:#04x}\t-{} register 2- ".format(daddr_reg2, drp_reg2, clkout.upper()))
            ...
            log.info('DADDR_{}: {:#04x}\t-{} Register 2- '.format(daddr_reg2, drp_reg2, clkout.upper()))
            log.info(
                "DADDR_{}: {:#04x}\t-{} Register Shared with CLKOUT5- ".format(daddr_regshared, drp_regshared,
                                                                               clkout.upper()))
            return daddr_reg1, drp_reg1, daddr_reg2, drp_reg2, daddr_regshared, drp_regshared
        else:
            log.error(
                "\nERROR: Fractional divide setting only supported for CLKOUT0. Output clock set to {}\n".format(
                    clkout.upper()))
            sys.exit(-1)
    ...
    def drp_settings(self, m, d, phase, bw):
        ...
        if phase < 0:
            phase += 360.0
        data_m = self.drp_calc_m(m, phase)
        data_d = self.drp_calc_d(d)
        data_cpres = self.drp_cpres(m, bw)
        data_locking = self.drp_locking(m)
        return data_m, data_d, data_cpres, data_locking

    def w_clkout0_reg(self, regs, clk="ref_clk"):
        clk_lower = clk.lower()
        if clk_lower == "ref_clk":
            clk_reg_base = self.refclk_base
        else:
            clk_reg_base = self.dffclk_base
        ## Write to 0x08
        reg_name = clk_reg_base + "clkout0_reg1"
        node = self.hw.getNode(reg_name)
        node.write(regs[1])
        ## Write to 0x09
        reg_name = clk_reg_base + "clkout0_reg2"
        node = self.hw.getNode(reg_name)
        node.write(regs[3])
        if len(regs) == 6:
            ## Write to 0x07
            reg_name = clk_reg_base + "clkout0__shared_reg"
            node = self.hw.getNode(reg_name)
            node.write(regs[5])
        self.hw.dispatch()
        return True

    def w_clkfbout_reg(self, regs, clk="ref_clk"):
        clk_lower = clk.lower()
        if clk_lower == "ref_clk":
            clk_reg_base = self.refclk_base
        else:
            clk_reg_base = self.dffclk_base
        ## Write to 0x14
        reg_name = clk_reg_base + "clkfbout_reg1"
        node = self.hw.getNode(reg_name)
        node.write(regs[0])
        ## Write to 0x15
        reg_name = clk_reg_base + "clkfbout_reg2"
        node = self.hw.getNode(reg_name)
        node.write(regs[1])
        if len(regs) == 3:
            ## Write to 0x13
            reg_name = clk_reg_base + "clkfbout_shared_reg"
            node = self.hw.getNode(reg_name)
            node.write(regs[2])
        self.hw.dispatch()
        return True

    def w_div_reg(self, reg, clk="ref_clk"):
        clk_lower = clk.lower()
        if clk_lower == "ref_clk":
            clk_reg_base = self.refclk_base
        else:
            clk_reg_base = self.dffclk_base
        ## Write to 0x16
        reg_name = clk_reg_base + "div_reg"
        node = self.hw.getNode(reg_name)
        node.write(reg)
        self.hw.dispatch()
        return True

    def w_lock_reg(self, regs, clk="ref_clk"):
        clk_lower = clk.lower()
        if clk_lower == "ref_clk":
            clk_reg_base = self.refclk_base
        else:
            clk_reg_base = self.dffclk_base
        ## Write to 0x28
        reg_name = clk_reg_base + "power_reg"
        node = self.hw.getNode(reg_name)
        node.write(regs[0])
        ## Write to 0x18
        reg_name = clk_reg_base + "lock_reg1"
        node = self.hw.getNode(reg_name)
        node.write(regs[1])
        ## Write to 0x19
        reg_name = clk_reg_base + "lock_reg2"
        node = self.hw.getNode(reg_name)
        node.write(regs[2])
        ## Write to 0x1a
        reg_name = clk_reg_base + "lock_reg3"
        node = self.hw.getNode(reg_name)
        node.write(regs[3])
        self.hw.dispatch()
        return True

    def w_filter_reg(self, regs, clk="ref_clk"):
        clk_lower = clk.lower()
        if clk_lower == "ref_clk":
            clk_reg_base = self.refclk_base
        else:
            clk_reg_base = self.dffclk_base
        ## Write to 0x4e
        reg_name = clk_reg_base + "filter_reg1"
        node = self.hw.getNode(reg_name)
        node.write(regs[0])
        ## Write to 0x4f
        reg_name = clk_reg_base + "filter_reg2"
        node = self.hw.getNode(reg_name)
        node.write(regs[1])
        self.hw.dispatch()
        return True

    def w_drp_setting_reg(self, mult, divide, clk="ref_clk"):
        regs = self.drp_settings(mult, divide, 0, "opt")
        self.w_clkfbout_reg(regs[0], clk)
        self.w_div_reg(regs[1], clk)
        self.w_filter_reg(regs[2], clk)
        self.w_lock_reg(regs[3], clk)
        return True

    def set_clock(self, divide_O, mult, divide, clk="ref_clk"):
        clkout0_regs = self.drp_clkout(divide_O, 0.5, .0)
        self.w_clkout0_reg(clkout0_regs, clk)
        self.w_drp_setting_reg(mult, divide, clk)
        return True
